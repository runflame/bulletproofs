...
impl R1CSProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<R1CSProof, R1CSError> {
        ...
        let t_x = Option::<Scalar>::from(Scalar::from_canonical_bytes(read32!()))
            .ok_or(R1CSError::FormatError)?;
        let t_x_blinding = Option::<Scalar>::from(Scalar::from_canonical_bytes(read32!()))
            .ok_or(R1CSError::FormatError)?;
        let e_blinding = Option::<Scalar>::from(Scalar::from_canonical_bytes(read32!()))
            .ok_or(R1CSError::FormatError)?;
